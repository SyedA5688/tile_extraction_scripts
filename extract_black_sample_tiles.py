...
def load_and_extract_tiles(filename, TISSUE_THRESHOLD_PERCENTAGE):
    ...
    # Calculating cutoffs
    c0_cut_top = noise_c0[2] - 1.7 * np.sqrt(noise_c0[3])  # top cutoff is median of noise cluster (contains tissue)
    c1_cut_top = noise_c1[2] - 1.5 * np.sqrt(noise_c1[3])
    c2_cut_top = noise_c2[2] - 2.0 * np.sqrt(noise_c2[3])
    c0_cut_bot = tissue_c0[2] - 1.7 * np.sqrt(tissue_c0[3])
    c1_cut_bot = tissue_c1[2] - 1.5 * np.sqrt(tissue_c1[3])
    c2_cut_bot = tissue_c2[2] - 2.0 * np.sqrt(tissue_c2[3])
    ...
    for k in range(DISQUALIFY_BORDER, np_img.shape[0] - TILE_HEIGHT - DISQUALIFY_BORDER, TILE_HEIGHT):
        ...
        while i < (np_img.shape[1] - TILE_WIDTH - DISQUALIFY_BORDER):  # From 0 to width, inc by 50 at end
            ...
            for grid_line in range(GRID_INCREMENT, max_dim, GRID_INCREMENT):  # 50 up to max_dim, inc by 50
                for iterPt in range(0, max_dim):  # 0 up to max_dim, every pixel on line
                    ...
                    if grid_line < TILE_HEIGHT and iterPt < TILE_WIDTH:
                        ...
                        if (((tile[grid_line, iterPt, 0] < c0_cut_top) and (tile[grid_line, iterPt, 0] > c0_cut_bot))
                                and ((tile[grid_line, iterPt, 1] < c1_cut_top) and (tile[grid_line, iterPt, 1] > c1_cut_bot))
                                and ((tile[grid_line, iterPt, 2] < c2_cut_top) and (tile[grid_line, iterPt, 2] > c2_cut_bot))):
                            tissue_pixels += 1

                    if iterPt < TILE_HEIGHT and grid_line < TILE_WIDTH:
                        total_pixels += 1
                        # Pixel on vertical grid line; first is at 0 height, 50 right
                        if (((tile[iterPt, grid_line, 0] < c0_cut_top) and (tile[iterPt, grid_line, 0] > c0_cut_bot))
                                and ((tile[iterPt, grid_line, 1] < c1_cut_top) and (tile[iterPt, grid_line, 1] > c1_cut_bot))
                                and ((tile[iterPt, grid_line, 2] < c2_cut_top) and (tile[iterPt, grid_line, 2] > c2_cut_bot))):
                            tissue_pixels += 1
            ...
            if percentage > TISSUE_THRESHOLD_PERCENTAGE and decreased:
                ...
                #  plt.imsave('/home/cougarnet.uh.edu/pcicales/Desktop/Chang_set_1/TI_crops_set_a_v3/{}_x{}_y{}_{}.png'.format(filename, i, k, round(100*percentage, 2)), tile1_contig)
    ...
    if (num_saved_tiles < 8):
        ...
        thumbnailImg.save('/home/cougarnet.uh.edu/pcicales/Desktop/Chang_set_1/TI_noise_debugging_Syed/thumbnails/{}_thumbnail.png'.format(filename))
        print("Saved", num_saved_tiles, "tiles from", filename, ". Done, thumbnail photo in subfolder.\n")


if __name__ == "__main__":
    # Samples with black backgrounds that messed up main script clustering: Unknown(10.7.19)_2210_100720190105.tif
    wsi_filename = '/home/cougarnet.uh.edu/pcicales/Desktop/Chang_set_1/AC_set_a/Unknown(10.7.19)_2210_100720190105.tif'
    load_and_extract_tiles(wsi_filename, .65)  # Pass tissue threshold percentage as a decimal
